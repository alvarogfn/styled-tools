...
} from "@/styled-tools/types";
import { StyleFunction } from "styled-components";

type AnyFunction<Props = object, Return = any> = (props: Props) => Return;

function parseFunction<Props, Function extends AnyFunction<Props>>(
  props: Props,
  test: Function,
): boolean {
...
function parseObject<Props extends ComponentProps>(
  props: ComponentPropsWithTheme<Props>,
  test: object,
): boolean {
...
function parseString<Props extends ComponentProps>(
  props: ComponentPropsWithTheme<Props>,
  test: string,
): boolean {
...
export function ifProp<Props extends ComponentProps>(
  ...
  return (props: ComponentPropsWithTheme<Props>) => {
    ...
    if (Array.isArray(test)) {
      result = test.some((item: Needle<Props>) => {
